...
export function getRailroadCount(state: GameState, playerId: string): number {
  return RAILROAD_IDS.filter(
    (propertyId) =>
      state.deeds[propertyId].ownerId === playerId && !state.deeds[propertyId].mortgaged,
  ).length;
}

export function getUtilityCount(state: GameState, playerId: string): number {
  return UTILITY_IDS.filter(
    (propertyId) =>
      state.deeds[propertyId].ownerId === playerId && !state.deeds[propertyId].mortgaged,
  ).length;
}
...
export function getPropertyDetailsSummary(state: GameState, propertyId: number): string {
  ...
  if (space.type === "property") {
    const owner = deed.ownerId
      ? (getPlayerById(state, deed.ownerId)?.name ?? "Unknown")
      : "Unowned";
    const status = deed.mortgaged
      ? "Mortgaged"
      : deed.hotel
        ? "Hotel"
        : deed.houses > 0
